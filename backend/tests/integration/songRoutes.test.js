...
describe("Song APIs", () => {
  ...
  it("POST /api/songs requires auth", async () => {
    const res = await request(app)
      .post("/api/songs")
      .send({ title: "x", artist: "y", url: "http://example.com" });
  ...
  it("GET /api/songs?userId returns only that user songs in desc order", async () => {
    ...
    await prisma.song.create({
      data: { title: "Other", artist: "B", url: "c.mp3", userId: user2.id },
    });
  ...
  it("GET /api/songs proxies stored and public supabase URLs to /api/media", async () => {
    ...
    const publicUrl =
      "https://example.supabase.co/storage/v1/object/public/songs/path/to/public.mp3";
    await prisma.song.create({
      data: { title: "Public", artist: "Y", url: publicUrl, userId: user.id },
    });
    ...
    expect(
      foundLocal.url.startsWith("/api/media") ||
        foundLocal.url.includes("/api/media"),
    ).toBe(true);
    expect(
      foundPublic.url.startsWith("/api/media") ||
        foundPublic.url.includes("/api/media"),
    ).toBe(true);
  });
  ...
  it("POST /api/songs with token for missing user returns 404", async () => {
    ...
    const res = await request(app)
      .post("/api/songs")
      .set("Authorization", `Bearer ${token}`)
      .send({ title: "X", artist: "Y", url: "u.mp3" });
    expect(res.status).toBe(404);
  });
  ...
  it("GET /api/songs leaves non-supabase http URLs unchanged", async () => {
    ...
    await prisma.song.create({
      data: { title: "HttpSong", artist: "C", url: httpUrl, userId: user.id },
    });
  ...
  it("parses full /api/media URLs and proxies them to same-origin media path", async () => {
    ...
    const fullMediaUrl =
      "https://myhost/api/media?bucket=songs&path=path/to/media.mp3";
    await prisma.song.create({
      data: {
        title: "FullMedia",
        artist: "M",
  ...
  it("GET /api/songs/top/rated returns songs ordered by ratings", async () => {
    ...
    const s1 = await prisma.song.create({
      data: { title: "One", artist: "A", url: "1.mp3", userId: user.id },
    });
    const s2 = await prisma.song.create({
      data: { title: "Two", artist: "A", url: "2.mp3", userId: user.id },
    });
    ...
    const raterRecord1 = await prisma.user.findUnique({
      where: { email: rater1.email },
    });
    const raterRecord2 = await prisma.user.findUnique({
      where: { email: rater2.email },
    });
    // add ratings: s2 gets two ratings, s1 gets one
    await prisma.rating.create({
      data: { userId: raterRecord1.id, songId: s2.id, score: 5 },
    });
    await prisma.rating
      .create({ data: { userId: raterRecord1.id, songId: s1.id, score: 4 } })
      .catch(() => {});
    await prisma.rating
      .create({ data: { userId: raterRecord2.id, songId: s2.id, score: 5 } })
      .catch(() => {});

    const res = await request(app).get(
      `/api/songs/top/rated?limit=2&userId=${user.id}`,
    );
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
    expect(res.body.length).toBeGreaterThanOrEqual(2);
  ...
  it("DELETE invokes supabase.remove for both audio and image references", async () => {
    ...
    const song = await prisma.song.create({
      data: {
        title: "ToDeleteBoth",
        artist: "Del",
        url: "stored/audio.mp3",
        imageUrl: "stored/covers/image.png",
        userId: ownerRecord.id,
      },
    });
    ...
    const removedArgs = mockRemove.mock.calls.map((c) =>
      Array.isArray(c[0]) ? c[0][0] : c[0],
    );

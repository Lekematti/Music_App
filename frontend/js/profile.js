...
const renderProfileSongs = (publicationList, userSongs) => {
  ...
  if (userSongs.length === 0) {
    publicationList.innerHTML =
      '<p style="color: #888;">You haven\'t published any music yet.</p>';
    return;
  }
  ...
  userSongs.forEach((song) => {
    ...
    const statsText =
      ratingsCount > 0
        ? `Avg Rating: ★ ${avgRating} (${ratingsCount})`
        : "No ratings yet";
...
const getOverallAverageRating = (userSongs) => {
  ...
  return totals.totalRatings > 0
    ? (totals.totalScore / totals.totalRatings).toFixed(1)
    : "0.0";
};

const updateAvgRatingLabel = () => {
  const likesLabelEl = document.querySelector(
...
const loadUserProfile = async () => {
  try {
    ...
    const songsResponse = await fetch(
      `${API_BASE}/api/songs?userId=${userData.id}`,
    );
...
const initProfilePage = () => {
  ...
  if (deleteBtn && !deleteBtn.dataset.listenerAttached) {
    deleteBtn.addEventListener("click", async () => {
      const confirmed = globalThis.confirm(
        "Delete your account and all your songs? This cannot be undone.",
      );
      if (!confirmed) return;
  ...
  if (
    changeAvatarBtn &&
    avatarFileInput &&
    !changeAvatarBtn.dataset.listenerAttached
  ) {
  ...
  const tabIcons = {
    "publications-section": "../assets/icons/profile.png",
    "password-section": "../assets/icons/profile.png",
    "username-section": "../assets/icons/profile.png",
    "email-section": "../assets/icons/profile.png",
  };
  ...
  navItems.forEach((item) => {
    item.addEventListener("click", () => {
      ...
      if (activeImg) {
        activeImg.src =
          tabIcons[item.dataset.target] || "../assets/icons/settings.png";
      }
